...
function graphDistribution() {
    ...
    if (isIntegerArray(values)) {
        ...
        let sorted = Object.keys(data).sort().reduce((obj, key) => {
            obj[key] = data[key];
            return obj;
        }, {});
...
function graphArrayElement(loc, value, opacity) {
    ...
    cubes.push(cube);
    // Cube edges
    let wireframe = new THREE.LineSegments(new THREE.EdgesGeometry(geometry), new THREE.LineBasicMaterial({ color: 0x00ff00 }));
...
function yAxisLabels(loc, arr, font, doubleAxisSize) {
    ...
    if (shape.length == 2) {
        for (let i = 0; i < arr.length; i++) {
            ...
            let textsShapes = font.generateShapes(tickVal, 0.3);
            let textsGeometry = new THREE.ShapeBufferGeometry(textsShapes);
